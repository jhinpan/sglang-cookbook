    ...
    "configs": [
      ...
      },
      {
        ...
        "accuracy": [
          {
            "name": "GSM8K",
            "value": "97.7%",
            "note": "n=1319, chat+thinking; run_eval --eval-name gsm8k --thinking-mode glm-45 --max-tokens 8192 --temperature 0. REQUIRES all three gfx950 source fixes (two bpreshuffle disables + MQA env). Without the SECOND bpreshuffle disable (models/deepseek_common/utils.py, used by GLM-5.2's deepseek_v4 forward path), GSM8K collapses to 0.0 with token-garbage under batching. n=100 subset scored 98.0%.",
            "ref": "98.2% (cookbook ref); MI300X/gfx942 re-run 97.2% — gfx950 reaches parity once both bpreshuffle sites are fixed"
          },
          {
            "name": "AIME25",
            "value": "91.5%",
            "note": "pass@1 avg-of-16 via sgl-eval (NV official harness), n=30x16=480 samples; 95% CI 89.1-93.9 (SEM 1.22%, std 4.86%). pass@16 100%, majority@16 93.3%, truncated 0.21%, error 0%, 9.46M completion tokens. Requires all three gfx950 source fixes. Run with --n-repeats 16 --max-tokens 64000 --temperature 1.0 --top-p 0.95 --thinking. CAVEAT: always use sgl-eval, NOT in-tree run_eval (its strict Answer: first-match regex badly undercounts this thinking model). gfx950 reaches parity with gfx942 (90.6%) once the bpreshuffle sites are fixed.",
        ...
        "gotchas": [
          ...
          "Long-context fix: SGLANG_FP8_PAGED_MQA_LOGITS_TORCH must be 0. server_args.py force-set it True in the is_hip branch and EnvField.set() overwrites the shell export unconditionally, so a source patch (env-aware, default False) is needed.",
        ...
        "provenance": {
          "image": null,
          "pr": "https://github.com/sgl-project/sglang/pull/28471",
          "sglang": "0.5.13.post1.dev20260622 (g4923bb93ae), editable + 3 gfx950 source fixes (2x bpreshuffle disable + MQA env)",
